...
public class ChatFrame extends JFrame implements ActionListener, KeyListener {
	...
	private JButton register; //gumb, ki bo registriral uporabnika
	private JButton privat; //gumb, ki bo odprl okno, kjer bo uporabnik lahko doloèil komu kaj poslati
	...
	public void sendPrivMessage(String person, String recipient, String message) throws ClientProtocolException, URISyntaxException, IOException {
		if (message.isEmpty() == false) {
	...
	public void getMessages() {
		try {
			...
			for(int i = 0; i < list.length(); i++) {
				...
				if (sporocilo.getBoolean("global")) {
					this.addMessage(sporocilo.getString("sender"), sporocilo.getString("text"));
				} else {
					this.addMessage("Privatno:" + sporocilo.getString("sender") + " -> " + staro_ime, sporocilo.getString("text"));
				}
			}
	...
	public void keyPressed(KeyEvent arg0) {
		if (arg0.getKeyChar() == KeyEvent.VK_ALT) {
			System.out.println("Nekaj");
			try {
				this.sendPrivMessage(staro_ime, this.input.getText().split("\\:")[0], this.input.getText().split("\\:")[1]);
			} catch (ClientProtocolException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			} catch (URISyntaxException e1) {
